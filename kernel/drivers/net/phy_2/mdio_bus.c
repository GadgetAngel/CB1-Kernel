...
struct phy_device *mdiobus_get_phy(struct mii_bus *bus, int addr)
{
    ...
    if (!mdiodev)
    {
        printk("==> %s: mdiodev=0 ,error\n",__func__);
        return NULL;
    }

    if (!(mdiodev->flags & MDIO_DEVICE_FLAG_PHY))
    {
        printk("==> %s: mdiodev=0 MDIO_DEVICE_FLAG_PHY ,error\n",__func__);
        return NULL;
    }
 ...
 * Returns 0 on success or < 0 on error.
 */
int __mdiobus_register(struct mii_bus *bus, struct module *owner) //kandy
{
    struct mdio_device *mdiodev;
    int i, err;
  ...
  //  pr_info("==>%s: register OK, scan index = %d\n", bus->name,i);
    return 0;
...
struct phy_device *mdiobus_scan(struct mii_bus *bus, int addr)
{
	...
	if (err) {
       // printk("==> %s: phy_device_register error,addr = %d\n",__func__,addr);
		phy_device_free(phydev);
        
		return ERR_PTR(-ENODEV);
	}
    
   // printk("==> %s: scan OK ,addr = %d\n",__func__,addr);
