...
struct phy_device *get_phy_device(struct mii_bus *bus, int addr, bool is_c45)
{
	...
	if (r)
    {
      //  printk("==> %s: get_phy_id error phy_id = 0x%08x addr = %d\n",__func__,phy_id,addr);
		return ERR_PTR(r);
    }
...
{
	return get_phy_c45_ids(phydev->mdio.bus, phydev->mdio.addr,&phydev->c45_ids);
}
