...
int phy_config_aneg(struct phy_device *phydev)
{
    ...
    printk("==> %s: go to genphy_config_aneg\n",__func__);
...
static void HandleAnEvent(struct phy_device *phydev)
{
    int err=0;
    
        printk("==> %s: enter PHY_AN\n",__func__);
		err = phy_read_status(phydev);
		if (err < 0)
        {
            printk("==> %s: phy_read_status error\n",__func__);
			return ;
        }
#if 0
		...
		if (err < 0)
        {
            printk("==> %s: phy_aneg_done error\n",__func__);
			return;
        }

		/* If AN is done, we're running */
		if (err > 0) {
             printk("==> %s: PHY_RUNNING\n",__func__);
			phydev->state = PHY_RUNNING;
...
static int _phy_start_aneg(struct phy_device *phydev)
{
    ...
    {
        printk("==> %s: autoneg disable,phy_sanitize_settings\n",__func__);
	...
	if (err < 0)
    {
        printk("==> %s: phy_config_aneg error\n",__func__);
		return err;
    }
	...
	//if (phydev->state != PHY_HALTED) 
    {
        ...
        {
			phydev->state = PHY_AN;
            printk("==> %s: set state PHY_AN\n",__func__);
            HandleAnEvent(phydev);
    ...
    {
        printk("==> %s: phy is started,check link status\n",__func__);
