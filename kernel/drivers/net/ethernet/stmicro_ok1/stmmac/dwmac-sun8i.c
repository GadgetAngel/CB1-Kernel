...
static int sun8i_dwmac_init(struct platform_device *pdev, void *priv)
{
	...
	if (ret) {
        ...
        printk("==> enable emac1 clk OK\n");
    
	ret = clk_prepare_enable(gmac->ephy_clk); //kandy
	if (ret) {
		dev_err(&pdev->dev, "Could not enable ephy_clk clock\n");
		goto err_disable_regulator;
	}	
    else
        printk("==> enable ephy clk OK\n");
...
static int sun8i_dwmac_probe(struct platform_device *pdev)
{
	...
	gmac->tx_clk = devm_clk_get(dev, "gmac1_clk"); //kandy gmac->ephy_clk
	if (IS_ERR(gmac->tx_clk)) {
		dev_err(dev, "Could not get TX clock\n");
		return PTR_ERR(gmac->tx_clk);
	}
	
	gmac->ephy_clk = devm_clk_get(dev, "ephy_25m_clk"); //kandy gmac->ephy_clk
	if (IS_ERR(gmac->ephy_clk)) {
		dev_err(dev, "Could not get ephy clock\n");
		return PTR_ERR(gmac->ephy_clk);
	}
