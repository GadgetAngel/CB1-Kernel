...
static int geth_power_on(struct geth_priv *priv)
{
    ...
    printk("==> geth_power_on, read phy reg = %08x\n",value);
	...
	printk("==> geth_power_on, set phy reg = %08x\n",value);
...
static int geth_phy_init(struct net_device *ndev)
{
   ...
   // if( __mdiobus_register(new_bus, THIS_MODULE) )
    ...
    ac300_id_h = mdiobus_read(new_bus,16,2);
    mdiobus_read(new_bus,16,3);
    
    if(ac300_id_h !=0xc000 )
    {
        printk("===> %s: AC300 id is not right,exit\n",__func__);  
        return -1;
    }
    
    printk("===> %s: start scan phy 12\n",__func__); 
#if 0   
...
#if 1  
  //  struct phy_device *phydev_tmp=NULL;

	{
		...
		for (addr = 0; addr < PHY_MAX_ADDR; addr++) {
           ...
           // printk("==> phydev_tmp->phy_id = %08x addr = %d ,get phy...\n",phydev_tmp->phy_id,addr);
...
static void geth_clk_enable(struct geth_priv *priv)
{
   ...
   // clk_set_rate(priv->geth_clk, 200000000);
	if (clk_prepare_enable(priv->geth_clk)) pr_err("try to enable geth_clk failed!\n");
    
    
    clock_freq = clk_get_rate(priv->geth_clk);
    printk("==> get emac1 clk = %d\n",clock_freq);
    
#if 1   

	clk_set_rate(priv->ephy_clk, 25000000);
    ...
    clock_freq = clk_get_rate(priv->ephy_clk);
    printk("==> get ephy clk = %d\n",clock_freq);    
    
	phy_interface = priv->phy_interface;

	clk_value = readl(priv->base_phy);
    printk("==> -------------------- read ephy = %08x\n",clk_value);
...
static int geth_open(struct net_device *ndev)
{
	...
	if (ret) {
		ret = -EINVAL;
        printk("==> %s: geth_dma_desc_init error\n",__func__);
		return ret;
	}
...
#if 1
    ret = gpio_request(GPIO_SYS_LED,"SysLed");
    if(ret)
